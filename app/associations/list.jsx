...
import { ASSOC_DATA } from "@/constants/Resources";
...
const ListLayout = () => {
  ...
  const [searchResults, setSearchResults] = useState(ASSOC_DATA);
  ...
  const handleSearch = (query) => {
    if (query) {
      const filteredData = ASSOC_DATA.filter((item) =>
        item.name.toLowerCase().includes(query.toLowerCase())
      );
      setSearchResults(filteredData);
    } else {
      setSearchResults(ASSOC_DATA);
    }
  };
  ...
  const handleFilter = async () => {
    const result = await navigateForResult(setCallback, router, "menus/filter");
